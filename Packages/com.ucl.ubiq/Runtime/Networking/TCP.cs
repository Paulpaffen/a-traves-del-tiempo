...
namespace Ubiq.Networking
{
    ...
    public class TCPConnection : INetworkConnection
    {
        ...
        {
            // If the host is already an IP literal (e.g. "192.168.0.101") skip the
            // DNS lookup entirely. Dns.GetHostEntry performs a *reverse* (PTR) DNS
            // query even for IP literals, which blocks the calling (main) thread
            // until it times out on networks with no reachable DNS server (an
            // isolated, offline LAN). This caused ~30-45s freezes / reconnect loops
            // on Quest. IPAddress.TryParse avoids any network call for IP literals.
            if (IPAddress.TryParse(host, out var literalIp))
            {
                return new IPEndPoint(literalIp, int.Parse(port));
            }
